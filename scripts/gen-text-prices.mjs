...
//
// What we keep: chat models from the native vendors ai-lcr documents
// (openai · anthropic · gemini · deepseek · xai · mistral), keyed by the BARE
// model id the user passes to that vendor's AI SDK provider (e.g.
// `anthropic("claude-haiku-4-5")`, `google("gemini-2.5-flash")`). Per-token
// prices are converted to ai-lcr's unit: USD per 1M tokens.
//
...
// Native model makers only — a vendor's own API is the cheapest, most-featureful
// route, and the one whose price is a stable list rate worth bundling.
const ALLOW = new Set(["openai", "anthropic", "gemini", "deepseek", "xai", "mistral"]);
...
// gemini · deepseek · xai · mistral), keyed by the BARE model id you pass to that
// vendor's AI SDK provider. USD per 1M tokens (input / output, optional cacheRead).
// This is the "go direct" rate — the cheapest, most-featureful route. Aggregator
// discounts (Kunavo, TokenMart, …) are NOT baked in; express them per-entry with
// \`discount\` (see createLCR's autoPrice). Refresh: node scripts/gen-text-prices.mjs
